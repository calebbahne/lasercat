...
bool homeSteppers() {
  ...
  xyPos[0] = 1000; //Homing Offset
  xyPos[1] = 300; //Homing offset
  bothSteppers.moveTo(xyPos);
  lcd.clear();
  lcd.print(F("Steppers Homed"));
  delay(1000);
  for(int i = 0; i < max(xyPos[0], xyPos[1]); i++){
    bothSteppers.moveTo(xyPos);
    bothSteppers.run();
  }
...
void executeCatAction(int selectedCat) {
  ...
  switch (selectedCat) {
    ...
    case 1:
      ...
      generatePath(100, 20, 20, 600, 2000); // (int segAmt, int numCurvPts, int numStrtPts, int speed, int stopTime, Point* start = nullptr)
      break;
    case 2:
...
bool executeCustomPath(){
  ...
  while(!isButtonReleased()){            //Change this so it can exit this part
    ...
    if (abs(xJoyVal - 512) < 45){
      joystickSpeedX = 0;
      if(outOfBounds()){
        moveSteppers(xStepper.currentPosition(),0);
      }
    }
    if (abs(yJoyVal - 512) < 45){
      joystickSpeedY = 0;
      if(outOfBounds()){
        moveSteppers(0,yStepper.currentPosition());
      }
    }
