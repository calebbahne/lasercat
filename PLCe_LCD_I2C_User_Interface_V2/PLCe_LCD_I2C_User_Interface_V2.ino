//This code provides the outline for the user interface, allowing the user to interact with the
// CatLaser using a joystick.
//Note: A lot of this was generated with ChatGPT, then troubleshooted from there. Hence the structures, pointers, and enums
//We'll fill in the functions and the rest of this.
...
// Menu options
const String menuOptions[] = {//an array filled with the display menus
  "< Random  Path >",
  "<    Sleep     >",
  "<Set Boundaries>",
  "<   Schedule   >",
...
const int numOptions = sizeof(menuOptions) / sizeof(menuOptions[0]); //Divides the size of the menuOptions array by the size of each array to find the numb of opts.
int currentOption = 0;
...
// Current mode function pointer (Saw this online)
...
// Function prototypes for modes: These are what we need to update
bool loadRandomPath();
bool loadCustomPath();
bool loadSchedule();
bool setBoundaries();
...
void setup() {
  ...
  int titleStart = (16 - title.length()) / 2; //This centers the title
...
void loop() {//The loop checks to see what the interface should be doing at each moment. It'll call other functions as needed. 
//We can leave this section alone for now, and just mess with the functions.
...
JoystickPositions readJoystickPositions() {
  JoystickPositions joyPos;
  joyPos.x = analogRead(joyXpin);
  joyPos.y = analogRead(joyYpin);
  return joyPos;
}

// Function to check if joystick was moved (for waking up from low power)
bool checkJoystickMoved() {
  JoystickPositions joyPos = readJoystickPositions();

  // Define movement thresholds
  // Consider movement if X or Y changes beyond a threshold from center (e.g., 512)
  if (joyPos.x < 200 || joyPos.x > 800 || joyPos.y < 200 || joyPos.y > 800) {
    return true;
  }
  return false;
}
