...
void generatePath(Point* start = nullptr) {
  ...
  const int NUM_CURVE_POINTS = 20; // Number of points for curved segments 20F
...
//Changes:
// 1. Put .run() multiple times throughout each function call
// 2. Test the number of times .run() needs to be called for continuous movement (make a counter)
// 3. Set the starting position to be (0,0) so that the steppers aren't behind from the very start.
// 4. Rewrite the code so that current center? is based on .currentPosition() of stepper motors
...
//I need to call run as much as I am currently, but need to send less points
//Even with speed, too many